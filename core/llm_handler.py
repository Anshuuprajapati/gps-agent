...
def classify_global_intent(current_state: str, user_message: str, conversation_context: str = "") -> str:
    ...
    result = extract_structured(
        ...
        "the driver).\n"
        "DRIVER_HANDOFF — the user wants the conversation/troubleshooting "
        "handed off to the driver so the driver deals with it directly, "
        "in any phrasing or spelling ('driver se baat kar lo', 'driver sa "
        "baat karo', 'driver ko de do', 'driver handle karega', 'driver "
        "ko bol do'). This is about transferring WHO is talking to the "
        "bot — do NOT use this for DIRECT_TECH_DISPATCH (sending an "
        "external technician to physically fix something) or "
        "DRIVER_UPDATE (actually giving a name+phone).\n"
        "DRIVER_CHANGE — the user wants to replace/change the driver on "
        "file with a different one, without yet giving the new driver's "
        "details in this message (e.g. 'driver change karna hai', 'naya "
        "driver hai', 'ye driver sahi nahi hai'). If a name and phone "
        "number are already given in the same message, prefer "
        "DRIVER_UPDATE instead.\n"
        ...
        "BOOKING_CORRECTION — the user wants to change/correct a detail of "
        "a service ticket that has ALREADY been booked (date, time, "
        "location, or contact), in any phrasing, e.g. 'sorry kal kar do', "
        "'kl ka kr do service book', 'time badal do 5 baje', 'wrong "
        "location tha, Delhi kar do'. Only plausible when a booking/ticket "
